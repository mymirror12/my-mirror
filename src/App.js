...
const S = {
  ...
  scoreBg: { marginTop:'20px', textAlign:'center', backgroundColor:'#111', padding:'20px 40px', borderRadius:'20px', border:'1px solid #222' },
  ...
  feedbackBase: { marginTop:'15px', backgroundColor:'#1a1a2e', padding:'20px 30px', borderRadius:'15px', color:'#fff', fontSize:'1.1rem', textAlign:'center', maxWidth:'360px', lineHeight:'1.5' },
  ...
  downloadLink: { display:'block', marginTop:'8px', color:'#7c3aed', fontSize:'0.85rem' },
...
const moodData = {
  happy:     { msg:'Waah! Aaj tu bahut khush lag raha hai! Yeh energy pure din rakhna!', sc:95, tips:['😊 Positivity doston ke saath share karo','🌟 Naya kaam shuru karne ka perfect time','💪 Gym ya walk ke liye best mood!'], color:'#00ff88' },
  sad:       { msg:'Thoda sad lag raha hai — koi baat nahi, kal better hoga!', sc:55, tips:['🎵 Apni favorite song suno abhi','☕ Garam chai piyo aur rest karo','📞 Kisi dost ko call karo'], color:'#4376FF' },
  angry:     { msg:'Stress dikh raha hai — thodi deep breathing karo!', sc:50, tips:['🧘 5 min deep breathing karo','🚶 Bahar short walk pe jao','💧 Thanda paani piyo aur relax karo'], color:'#ff4444' },
  surprised: { msg:'Surprised expression! Kya hua? Excitement feel ho rahi hai!', sc:70, tips:['🎯 Is energy ko goal pe lagao','✨ Naya experiment karo aaj','📸 Yeh moment capture karo!'], color:'#ffaa00' },
  fearful:   { msg:'Nervous mat ho — tu capable hai, doubt mat kar!', sc:60, tips:['💪 Khud pe trust karo','📝 Feelings paper pe likho','🌬️ Slow breathing se anxiety kam hogi'], color:'#ff6b35' },
  disgusted: { msg:'Kuch bura feel ho raha hai — fresh start karte hain!', sc:55, tips:['🌅 Fresh air lo bahar','🍎 Healthy kuch khao','🎮 Thoda break lo, mind reset hoga'], color:'#ff4444' },
  neutral:   { msg:'Neutral mode mein hai — thoda smile karo, sab better lagega!', sc:75, tips:['😄 Mirror mein smile practice karo','🎯 Aaj ka ek small goal set karo','☀️ Dhoop mein 10 min baitho'], color:'#ffaa00' },
};
...
export default function App() {
  ...
  const saveToHistory = (sc, emo) => {
    const now = new Date();
    const entry = { date: now.toLocaleDateString('en-IN'), time: now.toLocaleTimeString('en-IN', { hour:'2-digit', minute:'2-digit' }), score: sc, emotion: emo };
  ...
  const analyzeMe = async () => {
    ...
    try {
      ...
      if (!detection) {
        setFeedback('Face nahi dikh raha — achhi roshni mein camera ke saamne aao!');
        setAnalyzing(false);
        return;
      }

      const dominant = Object.entries(detection.expressions).sort((a, b) => b[1] - a[1])[0][0];
  ...
  const getScoreStyle = (sc) => {
    const val = sc !== undefined ? sc : score;
    if (val >= 85) return S.scoreHigh;
    if (val >= 70) return S.scoreMid;
    return S.scoreLow;
  };
  ...
  return (
    <div style={S.wrap}>
      ...
      <p style={S.sub}>Real AI face and emotion analysis</p>
      ...
      <div style={S.btnWrap}>
        {!started ? (
          ...
          <>
            ...
            <button onClick={speaking ? stopSpeaking : () => speak(feedback)} style={S.voiceBtn} disabled={!feedback}>
              {speaking ? 'Stop' : 'Suno'}
            </button>
          </>
        )}
      ...
      {photo && (
        <div style={S.photoWrap}>
          ...
          <a href={photo} download="dailymirror.png" style={S.downloadLink}>Photo Download Karo</a>
        </div>
      )}
      ...
      {history.length > 0 && (
        <div style={S.historyWrap}>
          ...
          <button onClick={() => { setHistory([]); localStorage.removeItem('mirrorHistory'); }} style={S.clearBtn}>
            History Clear
          </button>
        </div>
      )}
